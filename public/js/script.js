...
<div class="row">
			...
			<div class="card-body p-6">
				...
				<div class="form-group">
					...
					<label class="custom-switch">
						<input type="checkbox" class="custom-switch-input" name="private" value="1" ${!a?"checked":""}>
	...
	`);i.find('[name="name"]').focus(),(new d).assign({form:i.find("form"),fields:{name:{required:!0}},callback:function(t){o("account_category_edit"),n&&n()},confirmMessage:"save?"}),window.app.readJson(window.app.account.dir,function(s){let a={0:"-"};for(let e in s.categories){let n=s.categories[e];(!t||n.id!=t&&n.parent_id!=t)&&(a[n.id]=n.name)}i.find('[name="parent_id"]').setSelectOption(a,e)})},h=function({categoryId:t=0}){let e=l("account_category_list",escapeTemplate`
<div class="row">
	<div class="col col-lg-9 mx-auto">
		<form class="card">
			<div class="card-header">
...
<div class="row test-row-end mt-5">
	...
	`);S("#test-select-column1",test.col1,-1),S("#test-select-column2",test.col2,test.col1),test.title.length<=2&&a.find("#test-select-column2").hide(),a.find("#test-select-column1").change(()=>{P(parseInt($("#test-select-column1").val())),_()}),a.find("#test-select-column2").change(()=>{test.col2=parseInt($("#test-select-column2").val()),_()}),a.find('[name="test-retry"]').click(()=>{_()}),a.find('[name="test-reverse"]').click(()=>{P(test.col2),_()}),a.find('[name="test-before"]').click(()=>{I()}),a.find('[name="test-next"]').click(()=>{M()}),a.find('[name="test-random"]').click(()=>{test.isRandom=!!$('[name="test-random"]').prop("checked"),_()}),a.find('[name="test-yesno"]').click(()=>{test.isYesNo=!!$('[name="test-yesno"]').prop("checked"),_()}),a.find('[name="test-mute"]').click(()=>{test.isMute=!!$('[name="test-mute"]').prop("checked"),window.app.cookies.set("mute",test.isMute?1:0)}),a.find('[name="test-search"]').change(()=>{test.searchText=$('[name="test-search"]').val(),_()}),a.find(".test-row-yesno button.test-yes").click(E),a.find(".test-row-yesno button.test-no").click(M),s&&document.addEventListener("keydown",function(t){test.test&&0!=$(".test").length&&(37==t.keyCode?I():39==t.keyCode?O():40==t.keyCode&&q())}),_()};function S(t,e,s){let a="";for(let t=0;t<test.title.length;t++)test.title[t]&&t!=s&&(a+=`<option value="${t}" ${t==e?'selected="selected"':""}>${test.title[t]}</option>`);$(t).html(a)}function P(t){let e=test.col1;test.col1=t,test.col2==test.col1&&(test.col2=e),S("#test-select-column1",test.col1,-1),S("#test-select-column2",test.col2,test.col1)}function _(){let t={},e=function(t){let e=[];for(let s=0;s<t.length;s++)e.push(t[s]);return e}(test.list);test.isRandom&&L(e),test.test=[];for(let s=0;s<e.length;s++){if(test.searchText){let t=!1;for(let a=0;a<test.title.length;a++)if(e[s][a]&&e[s][a].indexOf(test.searchText)>=0){t=!0;break}if(!t)continue}let a=e[s][test.col1];a&&(e[s][test.col2]&&(t[a]||(test.test.push({key:a,rows:[]}),t[a]=test.test.length),test.test[t[a]-1].rows.push(e[s])))}if(0==test.test.length)return void alert("no data");let s=test.title[test.col2];$(".test-prompt").html(s?s+" ?":"");for(let t=0;t<test.test.length;t++)test.test[t].index=t;test.pos=0,test.pages=[0,test.test.length],T()}function T(t,e){if(test.isAnswer=!1,e&&test.test.push(test.test[test.pos]),t&&(test.pos+=t,-1==t&&test.pos<test.test.length-1&&test.test[test.pos].key==test.test[test.test.length-1].key&&test.test.pop()),$(".test-row-answer").hide(),test.pos==test.test.length)test.isEnd=!0,$(".test-row-end").show(),$('[name="test-before"]').hide(),$(".test-row-question").hide(),$(".test-row-options").hide(),$(".test-row-yesno").hide(),test.pages[1]==test.test.length?($('[name="test-end-message"]').html('<span class="lang-msg-perfect"></span>'),test.isMute||A("audio/perfect.mp3")):($('[name="test-end-message"]').html('<span class="lang-msg-finished"></span>'),test.isMute||A("audio/end.mp3"));else{test.isEnd=!1,$(".test-question").html(test.test[test.pos].key),1==t&&test.pos==test.pages[test.pages.length-1]&&test.pages.push(test.test.length),-1==t&&test.pos==test.pages[test.pages.length-2]-1&&test.pages.pop();let e=test.pages.length-1,s="";for(let t=1;t<e;t++)s+=test.pages[t]-test.pages[t-1]+" ... ";let a=test.pages[e]-test.pages[e-1];if(s+="( "+(test.pos-test.pages[e-1]+1)+" / "+a+" )",test.test.length>test.pages[e]&&(s+=" ... "+(test.test.length-test.pages[e])),$('[name="test-page"]').html(s),test.isYesNo)$(".test-row-yesno").show(),$(".test-row-options").hide();else{$(".test-row-yesno").hide(),$(".test-row-options").show();let t=function(){let t,e=[],s=[],a=test.pages[1],n=test.test[test.pos].index,i={};for(let t=0;t<a;t++)test.test[t].index!=n&&e.push(t);L(e);let l=test.test[n].rows,o=null;for(let e=0;e<l.length;e++)(t=l[e][test.col2])&&(o||(o=t),i[t]=t);s.push({isAnswer:!0,label:o});for(let a=0;a<e.length;a++){l=test.test[e[a]].rows;let n=!1;for(let e=0;e<l.length;e++)if((t=l[e][test.col2])&&!i[t]){n=!0;break}if(n&&(s.push({label:t}),i[t]=t,s.length>=4))break}return L(s),s}(),e="";for(let s=0;s<t.length;s++){let a=t[s].label;e+=`<div class="col-md-6 mt-2">\n\t\t\t\t<button class="btn btn-secondary btn-lg btn-block ${t[s].isAnswer?"test-yes":"test-no"}">${a}</button>\n\t\t\t\t</div>`}$(".test-row-options").html(e),$(".test-row-options button.test-yes").click(E),$(".test-row-options button.test-no").click(M)}$(".test-row-question").show(),test.pos>0?$('[name="test-before"]').show():$('[name="test-before"]').hide(),$(".test-row-end").hide()}}function I(){test.isPreview||(test.isAnswer?T():test.pos>0&&T(-1))}function M(){O(!0)}function O(t){(test.isYesNo||t||test.isAnswer)&&(test.isEnd||(test.isAnswer?T(1,!0):function(){test.isAnswer=!0;let t=`<tr><th>${test.title[test.col1]}</th><td>${test.test[test.pos].key}</td></tr>`,e=test.test[test.pos].rows;for(let s=0;s<e.length;s++)if(e[s][test.col2]){t+=`<tr><th>${test.title[test.col2]}</th><td>${e[s][test.col2]}</td></tr>`;for(let a=0;a<test.title.length;a++)e[s][a]&&a!=test.col1&&a!=test.col2&&(t+=`<tr><th>${test.title[a]||""}</th><td>${e[s][a]}</td></tr>`);break}$(".test-row-answer table").html(t),$(".test-row-answer").show(),$('[name="test-before"]').show(),$(".test-row-question").hide(),$(".test-row-options").hide(),$(".test-row-yesno").hide()}()))}function E(){q(!0)}function q(t){(test.isYesNo||t)&&(test.isAnswer||test.isEnd||(test.isMute||A("audio/coin.mp3"),test.isYesNo?T(1):($(".test-question").html('<span class="test-ok"><i class="fe fe-check-circle"></i></span>'),setTimeout(function(){T(1)},1e3))))}function A(t){let e=new Audio;e.src=window.app.adjustUrl(t),e.volume=.5,e.play()}function L(t){for(let e=t.length-1;e>0;e--){let s=Math.floor(Math.random()*(e+1)),a=t[e];t[e]=t[s],t[s]=a}}var N=function({title:t,words:e,description:s,category:a,isPublic:n,tableId:i}){e||(e="English,Spanish\nHello,Hola\nGood morning,Buenos días\nGoodbye,Adiós\nThank you,Gracias\nSorry,Lo siento\nI don't know,No lo sé");let o=l("edit",escapeTemplate`
<div class="row">
	<div class="col-12">
	...
	`),c=o.find('[name="jexcel"]'),r=(new k).parse(e),p=[];r||(r=[["question","answer"]]);for(let t=0;t<r[0].length;t++)p.push({type:"text",width:200});let u=jspreadsheet(c[0],{data:r,columns:p});r[r.length-1][0]&&u.insertRow(),i||o.find('[name="control-overwrite"]').hide();let m=new d;m.assign({form:o.find("form"),fields:{title:{required:!0}},validate:function(t){return!!(r=(new k).validate(u.getData(),m))&&(window.app.cookies.get("auth")?(o.find('[name="words"]').val((new k).array2text(r)),!0):(y(),!1))},callback:function(t){t.table_path&&window.app.goto({url:t.table_path,cb:!0,post:!0})},confirmMessage:"save?"}),o.find('[name="btn-test"]').click(t=>{t.preventDefault(),t.stopPropagation();let e=o.find('[name="title"]').val(),s=(new k).validate(u.getData(),m);s&&j({title:e,list:s})}),window.app.account&&window.app.account.dir&&window.app.readJson(window.app.account.dir,function(t){if(t.categories){let e="";for(let s in t.categories){let a=t.categories[s].name;e+=`<a class="dropdown-item" href="javascript:void(0);" onclick="$(this).parent().parent().parent().find('input').val('${a}');">${a}</a>`}e&&o.find('[name="category"]').parent().append(`\n<div class="input-group-append">\n\t<button data-toggle="dropdown" type="button" class="btn btn-secondary dropdown-toggle"></button>\n\t<div class="dropdown-menu dropdown-menu-right">${e}</div>\n</div>\n\t\t\t\t\t`)}})},J=function({list:t,userId:e=0}){let s="";if(!t)return s;for(let a in t){let n=t[a],i="";n.description&&(i+=`<div class="text-muted">${n.description}</div>`),!e&&n.user_url&&(i+=`\n<div class="d-flex align-items-center${n.description?" pt-5":""}">\n\t<div class="avatar avatar-sm mr-2"><i class="fe fe-user"></i></div>\n\t<div>\n\t\t<a href="${n.user_url}" class="text-default">${n.user_name}</a>\n\t</div>\n</div>\n\t\t\t`),i&&(i=`<div class="card-body d-flex flex-column">${i}</div>`),s+=`\n<div class="col-md-6 col-lg-4">\n\t<div class="card">\n\t\t<div class="card-header">\n\t\t\t<h3 class="card-title"><a href="${n.url}">${n.name}</a></h3>\n\t\t</div>\n\t\t${i}\n\t</div>\n</div>\n\t\t`}return s};function C(t){let e=[];for(let s in t){let a=t[s];a.id=parseInt(a.id),e.push(a)}return e.sort(function(t,e){return t.id>e.id?-1:t.id<e.id?1:0}),e}function D(t,e){let s="",a=e[t.id];if(a){for(let t in a)s+=D(a[t],e);s=`<div class="card-body d-flex flex-column">${s}</div>`}return`\n\t<div class="card">\n\t\t<div class="card-header">\n\t\t\t<h3 class="card-title"><a href="${t.url}">${t.name}</a></h3>\n\t\t</div>\n\t\t${s}\n\t</div>\n\t`}var U=function(){let t=$("#main-container"),e=t.find('[name="user_id"]').val(),s=t.find(".user").text(),a=t.find(".user").attr("href"),n=t.find('[name="category_id"]').val(),i=t.find(".category").text(),l=t.find(".category").attr("href"),o=t.find('[name="table_id"]').val(),r=parseInt(t.find('[name="private"]').val()),d=t.find(".title").text(),p=t.find(".description").text(),u=t.find(".words").text(),m=encodeURIComponent(document.URL),f=e==window.app.account.id,g=(new k).parse(u);t=c(escapeTemplate`
<div class="row">
	<div class="col-lg-9">
		<div class="card">
			<div class="card-header">
			...
			<div class="card-body">
				...
				<div class="table-responsive mt-2">${{raw:function(t){if(!t)return"";let e='<table class="table mb-0 table-bordered">',s=0;for(let a=0;a<t.length;a++){0==a&&(e+='<thead class="thead-light">');let n=t[a],i=0==a?"th":"td";e+="<tr>",0==a&&(s=n.length);let l=Math.max(s,n.length);for(let t=0;t<l;t++){let s=t<n.length?n[t]:"";e+=`<${i}>${s}</${i}>`}e+="</tr>",0==a&&(e+="</thead>")}return e+="</table>"}(g)}}</div>
			</div>
		</div>
	</div>
	<div class="col-lg-3">
...
<div class="row" id="row-tables"></div>
	`),window.app.readJson(a,function(s){t.find("#a-user").text(s.name);let a="",n={};for(let t in s.categories){let e=s.categories[t].parent_id;e>0&&(n[e]||(n[e]=[]),n[e].push(s.categories[t]))}for(let t in s.categories)s.categories[t].parent_id>0||(a+=`\n<div class="col-md-6 col-lg-4">\n\t${D(s.categories[t],n)}\n</div>\n\t\t\t`);(a+=J({list:s.tables,userId:e}))||(a='<span class="lang-msg-no-data"></span>'),t.find("#row-tables").html(a)})}(),"127.0.0.1"!=location.hostname&&window.app.includeScript("https://mochihashi.github.io/static/pixel/tracking.js")})}]);