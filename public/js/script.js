...
<div class="row test-row-end mt-5">
	...
	`);S("#test-select-column1",test.col1,-1),S("#test-select-column2",test.col2,test.col1),test.title.length<=2&&s.find("#test-select-column2").hide(),s.find("#test-select-column1").change(()=>{_(parseInt($("#test-select-column1").val())),P()}),s.find("#test-select-column2").change(()=>{test.col2=parseInt($("#test-select-column2").val()),P()}),s.find('[name="test-retry"]').click(()=>{P()}),s.find('[name="test-reverse"]').click(()=>{_(test.col2),P()}),s.find('[name="test-before"]').click(()=>{I()}),s.find('[name="test-next"]').click(()=>{M()}),s.find('[name="test-random"]').click(()=>{test.isRandom=!!$('[name="test-random"]').prop("checked"),P()}),s.find('[name="test-yesno"]').click(()=>{test.isYesNo=!!$('[name="test-yesno"]').prop("checked"),P()}),s.find('[name="test-mute"]').click(()=>{test.isMute=!!$('[name="test-mute"]').prop("checked"),window.app.cookies.set("mute",test.isMute?1:0)}),s.find('[name="test-search"]').change(()=>{test.searchText=$('[name="test-search"]').val(),P()}),s.find(".test-row-yesno button.test-yes").click(C),s.find(".test-row-yesno button.test-check").click(M),s.find(".test-row-yesno button.test-no").click(M),s.find(".card-options-maximize").click(()=>{s.find(".card").addClass("fullscreen"),s.find(".card-options-maximize").hide(),s.find(".card-options-minimize").show()}),s.find(".card-options-minimize").click(()=>{s.find(".card").removeClass("fullscreen"),s.find(".card-options-maximize").show(),s.find(".card-options-minimize").hide()}),a&&document.addEventListener("keydown",function(t){test.test&&0!=$(".test").length&&(37==t.keyCode?I():39==t.keyCode?O():40==t.keyCode&&E())}),P()};function S(t,e,a){let s="";for(let t=0;t<test.title.length;t++)test.title[t]&&t!=a&&(s+=`<option value="${t}" ${t==e?'selected="selected"':""}>${test.title[t]}</option>`);$(t).html(s)}function _(t){let e=test.col1;test.col1=t,test.col2==test.col1&&(test.col2=e),S("#test-select-column1",test.col1,-1),S("#test-select-column2",test.col2,test.col1)}function P(){let t={},e=function(t){let e=[];for(let a=0;a<t.length;a++)e.push(t[a]);return e}(test.list);test.isRandom&&q(e),test.test=[];for(let a=0;a<e.length;a++){if(test.searchText){let t=!1;for(let s=0;s<test.title.length;s++)if(e[a][s]&&e[a][s].indexOf(test.searchText)>=0){t=!0;break}if(!t)continue}let s=e[a][test.col1];s&&(e[a][test.col2]&&(t[s]||(test.test.push({key:s,rows:[]}),t[s]=test.test.length),test.test[t[s]-1].rows.push(e[a])))}if(0==test.test.length)return void alert("no data");let a=test.title[test.col2];$(".test-prompt").html(a?a+" ?":"");for(let t=0;t<test.test.length;t++)test.test[t].index=t;test.pos=0,test.pages=[0,test.test.length],T()}function T(t,e){if(test.isAnswer=!1,e&&test.test.push(test.test[test.pos]),t&&(test.pos+=t,-1==t&&test.pos<test.test.length-1&&test.test[test.pos].key==test.test[test.test.length-1].key&&test.test.pop()),$(".test-row-answer").hide(),test.pos==test.test.length)test.isEnd=!0,$(".test-row-end").show(),$('[name="test-before"]').hide(),$(".test-row-question").hide(),$(".test-row-options").hide(),$(".test-row-yesno").hide(),test.pages[1]==test.test.length?($('[name="test-end-message"]').html('<span class="lang-msg-perfect"></span>'),test.isMute||N("audio/perfect.mp3")):($('[name="test-end-message"]').html('<span class="lang-msg-finished"></span>'),test.isMute||N("audio/end.mp3"));else{test.isEnd=!1,$(".test-question").html(test.test[test.pos].key),1==t&&test.pos==test.pages[test.pages.length-1]&&test.pages.push(test.test.length),-1==t&&test.pos==test.pages[test.pages.length-2]-1&&test.pages.pop();let e=test.pages.length-1,a="";for(let t=1;t<e;t++)a+=test.pages[t]-test.pages[t-1]+" ... ";let s=test.pages[e]-test.pages[e-1];if(a+="( "+(test.pos-test.pages[e-1]+1)+" / "+s+" )",test.test.length>test.pages[e]&&(a+=" ... "+(test.test.length-test.pages[e])),$('[name="test-page"]').html(a),test.isYesNo)$(".test-row-yesno").show(),$(".test-row-options").hide(),$(".test-row-yesno .test-check").show(),$(".test-row-yesno .test-no").hide();else{$(".test-row-yesno").hide(),$(".test-row-options").show();let t=function(){let t,e=[],a=[],s=test.pages[1],n=test.test[test.pos].index,i={};for(let t=0;t<s;t++)test.test[t].index!=n&&e.push(t);q(e);let l=test.test[n].rows,o=null;for(let e=0;e<l.length;e++)(t=l[e][test.col2])&&(o||(o=t),i[t]=t);a.push({isAnswer:!0,label:o});for(let s=0;s<e.length;s++){l=test.test[e[s]].rows;let n=!1;for(let e=0;e<l.length;e++)if((t=l[e][test.col2])&&!i[t]){n=!0;break}if(n&&(a.push({label:t}),i[t]=t,a.length>=4))break}return q(a),a}(),e="";for(let a=0;a<t.length;a++){let s=t[a].label;e+=`<div class="col-md-6 mt-2">\n\t\t\t\t<button class="btn btn-outline-secondary w-100 btn-lg btn-block ${t[a].isAnswer?"test-yes":"test-no"}">${s}</button>\n\t\t\t\t</div>`}$(".test-row-options").html(e),$(".test-row-options button.test-yes").click(C),$(".test-row-options button.test-no").click(M)}$(".test-row-question").hide().fadeIn(1500),test.pos>0?$('[name="test-before"]').show():$('[name="test-before"]').hide(),$(".test-row-end").hide()}}function I(){test.isPreview||(test.isAnswer?T():test.pos>0&&T(-1))}function M(){O(!0)}function O(t){(test.isYesNo||t||test.isAnswer)&&(test.isEnd||(test.isAnswer?T(1,!0):function(){test.isAnswer=!0;let t=`<tr><th>${test.title[test.col1]}</th><td>${test.test[test.pos].key}</td></tr>`,e=test.test[test.pos].rows;for(let a=0;a<e.length;a++)if(e[a][test.col2]){t+=`<tr><th>${test.title[test.col2]}</th><td>${e[a][test.col2]}</td></tr>`;for(let s=0;s<test.title.length;s++)e[a][s]&&s!=test.col1&&s!=test.col2&&(t+=`<tr><th>${test.title[s]||""}</th><td>${e[a][s]}</td></tr>`);break}$(".test-row-answer table").html(t),$(".test-row-answer").show(),$('[name="test-before"]').show(),$(".test-row-question").hide(),$(".test-row-options").hide(),test.isYesNo?($(".test-row-yesno .test-check").hide(),$(".test-row-yesno .test-no").show(),$(".test-row-answer .btn-list").hide()):($(".test-row-yesno").hide(),$(".test-row-answer .btn-list").show())}()))}function C(){E(!0)}function E(t){(test.isYesNo||t)&&(test.isEnd||(test.isMute||N("audio/coin.mp3"),test.isYesNo?T(1):($(".test-question").html('<span class="test-ok"><i data-feather="check-circle" style="width:48px;height:48px;"></i></span>'),feather.replace(),setTimeout(function(){T(1)},1e3))))}function N(t){let e=new Audio;e.src=window.app.adjustUrl(t),e.volume=.5,e.play()}function q(t){for(let e=t.length-1;e>0;e--){let a=Math.floor(Math.random()*(e+1)),s=t[e];t[e]=t[a],t[a]=s}}var A=function({title:t,words:e,description:a,category:s,isPublic:n,tableId:i,imageFile:o}){e||(e="English,Spanish\nHello,Hola\nGood morning,Buenos días\nGoodbye,Adiós\nThank you,Gracias\nSorry,Lo siento\nI don't know,No lo sé");let c=l("edit",escapeTemplate`
<div class="page-body">
  <div class="row row-cards">
	<div class="col-12">
...
<div class="page-body">
  <div class="row row-cards" id="row-tables"></div>
	...
	`),window.app.readJson(s,function(a){t.find("#a-user").text(a.name);let s="",n={};for(let t in a.categories){let e=a.categories[t].parent_id;e>0&&(n[e]||(n[e]=[]),n[e].push(a.categories[t]))}for(let t in a.categories)a.categories[t].parent_id>0||(s+=`\n<div class="col-md-6 col-lg-4">\n\t${J(a.categories[t],n)}\n</div>\n\t\t\t`);(s+=L({list:a.tables,userId:e}))||(s='<span class="lang-msg-no-data"></span>'),t.find("#row-tables").html(s).addClass("row-cards")})}(),"127.0.0.1"!=location.hostname&&"mem.local"!=location.hostname&&window.app.includeScript("https://mochihashi.github.io/static/pixel/tracking.js");let e=0,a=function(){window.feather?feather.replace():e++<10&&(console.log("feather is not loaded yet: "+e),setTimeout(a,100))};a()})}]);